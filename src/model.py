...
class BERTEmbedding(nn.Module):
    def __init__(self, vocab_size, emb_size):
        ...
        self.pos_emb = nn.Embedding(vocab_size, emb_size) # This can be Sinusiodal Positional encoding, numeric positions, relative positional...
        self.segment_emb = nn.Embedding(vocab_size, emb_size)
    
    def forward(self, x):
        sentence_size = x.size(1)
        pos_emb_tensor = torch.arange(sentence_size, dtype=torch.long).to(device) # We want tensor by sentence size, Q: wouldn't this always be the optimal length? => yes
        ...
        total_input_emb = self.token_emb(x) + self.pos_emb(pos_emb_tensor) + self.segment_emb(seg_emb_tensor.long())
...
class BERTModel(nn.Module):
    ...
    """
    def __init__(self, vocab_size, emb_size, n_layers, n_heads, dense_dim, dropout_rate):
        super(BERTModel, self).__init__()
        self.embedding = BERTEmbedding(vocab_size, emb_size)
    ...
    def forward(self, x, atten_pad_mask):
        embedding = self.embedding(x) # shape: bsz, seq_len, emb_size
